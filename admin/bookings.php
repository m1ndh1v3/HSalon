<?php
// ==========================
// /admin/bookings.php (safe redirect version)
// ==========================
...
// === Handle approve/cancel actions BEFORE output ===
if (isset($_GET['approve'])) {
    $id = intval($_GET['approve']);
    $pdo->prepare("UPDATE bookings SET status='approved' WHERE id=?")->execute([$id]);
    add_notification('booking', "تمت الموافقة على الحجز رقم #$id");
...
if (isset($_GET['cancel'])) {
    ...
    add_notification('booking', "تم إلغاء الحجز رقم #$id");
    header("Location: bookings.php?msg=cancelled");
    exit;
}

// === Only include header after all redirects are done ===
...
if ($statusFilter && in_array($statusFilter, ['pending','approved','cancelled'])) {
    $query .= " AND b.status = ?";
    $params[] = $statusFilter;
}
if ($dateFilter) {
...
<h2 class="text-center mb-4">إدارة الحجوزات</h2>

<!-- Summary bar -->
<div class="row mb-4 text-center">
  <div class="col-md-3 mb-2"><span class="badge bg-secondary fs-6">إجمالي: <?= $count_all ?></span></div>
  <div class="col-md-3 mb-2"><span class="badge bg-success fs-6">تمت الموافقة: <?= $count_approved ?></span></div>
  <div class="col-md-3 mb-2"><span class="badge bg-warning text-dark fs-6">معلقة: <?= $count_pending ?></span></div>
  <div class="col-md-3 mb-2"><span class="badge bg-danger fs-6">ملغاة: <?= $count_cancelled ?></span></div>
</div>

<?php if ($clientId > 0): ?>
  <div class="alert alert-info text-center" dir="rtl">
    عرض الحجوزات الخاصة بالعميل رقم <strong><?= $clientId ?></strong>
...
<form method="get" class="row mb-3 text-end" dir="rtl">
  ...
  <div class="col-md-3 mb-2">
    ...
    <input type="text" name="search" value="<?php echo clean($search); ?>" class="form-control text-end" placeholder="ابحث بالاسم أو الخدمة أو رقم الحجز">
  </div>
...
<table class="table table-bordered table-striped text-center align-middle" dir="rtl">
  <thead class="table-light">
    <tr>
      ...
      <th><?= sort_link('client_name','اسم العميل',$sort,$order) ?></th>
      <th><?= sort_link('service_name','الخدمة',$sort,$order) ?></th>
      <th><?= sort_link('date','تاريخ ووقت الموعد',$sort,$order) ?></th>
      <th><?= sort_link('created_at','تاريخ الحجز',$sort,$order) ?></th>
  ...
  <?php if ($rows): ?>
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body text-center">
              هل أنت متأكد أنك تريد الموافقة على هذا الحجز (#<?= $r['id'] ?>)؟
            </div>
      ...
      <div class="modal fade" id="confirmCancel<?= $r['id'] ?>" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body text-center">
              هل أنت متأكد أنك تريد إلغاء هذا الحجز (#<?= $r['id'] ?>)؟
            </div>
            <div class="modal-footer justify-content-center">
    ...
    <?php endforeach; ?>
  <?php else: ?>
    <tr><td colspan="7" class="text-muted">لا توجد حجوزات مطابقة للمعايير.</td></tr>
  <?php endif; ?>
  </tbody>
</table>
</div>
